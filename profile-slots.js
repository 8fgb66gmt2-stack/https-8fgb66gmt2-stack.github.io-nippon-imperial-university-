...
function oldPrefix(n){let h=2166136261;for(let i=0;i<n.length;i++){h^=n.charCodeAt(i);h=Math.imul(h,16777619)}return'niu:p:'+('00000000'+(h>>>0).toString(16)).slice(-8)+':'}
function migrate(n){n=clean(n);if(!n)return;let b=P+encodeURIComponent(n)+'::',nest=b+b,old=oldPrefix(n),ks=[];for(let i=0;i<L.call(localStorage);i++){let k=N.key.call(localStorage,i);if(k&&(k.startsWith(nest)||k.startsWith(old)))ks.push(k)}ks.forEach(k=>{let t=k.startsWith(nest)?b+k.slice(nest.length):b+k.slice(old.length),v=N.getItem.call(localStorage,k);if(v!==null&&N.getItem.call(localStorage,t)===null)N.setItem.call(localStorage,t,v)});['niu_arg2_found','niu_arg2_stage1','niu_arg2_stage2','niu_arg2_stage3'].forEach(k=>{let v=N.getItem.call(localStorage,k),t=b+k;if(v!==null&&N.getItem.call(localStorage,t)===null)N.setItem.call(localStorage,t,v)})}
function activate(n){n=clean(n);if(!n)return false;let p=active();migrate(n);rawSet('niu_entrant_name',n);rawSet('niu_active_profile',n);rawSet('niu_profiles_version',V);if(p!==n)dispatchEvent(new CustomEvent('niu:profile-changed',{detail:{previous:p,current:n}}));return true}
if(clean(raw('niu_entrant_name')))rawSet('niu_active_profile',clean(raw('niu_entrant_name')));rawSet('niu_profiles_version',V);migrate(active());
...
function entranceFix(){let e=document.getElementById('entrance');if(!e)return;['#arg2-trigger','a[href="shiryo-shitsu.html"]','a[href="archive16.html"]'].forEach(s=>document.querySelectorAll(s).forEach(x=>{if(!e.classList.contains('hide')){x.dataset.niuHidden='1';x.style.setProperty('display','none','important');x.style.setProperty('pointer-events','none','important')}else if(x.dataset.niuHidden){x.style.removeProperty('display');x.style.removeProperty('pointer-events');delete x.dataset.niuHidden}}))}
